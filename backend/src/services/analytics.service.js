...
const getDashboardSummary = async () => {
  ...
  if (isUsingLocalDatabase()) {
    const [allEvents, allRegistrations, allNotifications] = await Promise.all([
      localStore.listEvents(),
      localStore.listRegistrations(),
      localStore.listNotifications(),
    ]);

    const events = allEvents
      .filter((event) => event.status !== 'draft')
      .sort((left, right) => new Date(left.startDate) - new Date(right.startDate));
    const registrations = allRegistrations.sort(
      (left, right) => new Date(right.createdAt) - new Date(left.createdAt),
    );
    const recentNotifications = allNotifications
      .sort((left, right) => new Date(right.createdAt) - new Date(left.createdAt))
      .slice(0, 3);
    const eventMap = new Map(allEvents.map((event) => [event._id, event]));
    ...
    const avgOccupancy =
      totalEvents === 0
        ...
        : toRoundedPercentage(
            (events.reduce(
              (total, event) => total + event.registeredCount / event.capacity,
              0,
            ) /
              totalEvents) *
              100,
    ...
    const completedRegistrations = registrations.filter((registration) => {
      const event = eventMap.get(registration.event);
      if (!event) {
        return false;
      }
      ...
      return (
        ...
        (event.status === 'completed' || new Date(event.endDate) <= now)
      );
    }).length;

    const completionBase = registrations.filter(
      (registration) => registration.checkedIn,
    ).length;
    const completionRate =
      completionBase === 0
        ? 0
        : Math.round((completedRegistrations / completionBase) * 100);
    ...
    const registrationTrend = [...registrationTrendMap.entries()].map(
      ([label, value]) => ({
        label,
        value,
      }),
    ...
    const attendanceTrend = [...registrationTrendMap.keys()].map((label) => ({
      label,
      value: registrations.filter((registration) => {
        if (!registration.checkedIn || !registration.checkedInAt) {
          return false;
        }

        return (
          new Date(registration.checkedInAt).toISOString().slice(0, 10) === label
        );
      }).length,
    }));

    const modeBreakdown = [...modeMap.entries()].map(([label, value]) => ({
      label,
      value,
    }));
